...
impl BMP180 {
    pub fn new() -> BMP180 {
        let mut tmpi2c = LinuxI2CDevice::new("/dev/i2c-1", ADDR).unwrap_or_else(|_| {
            panic!("Cannot create i2c device for bmp180");
        });

        let compensation_coeffs =
            match tmpi2c.smbus_read_i2c_block_data(Reg::FirstCoeffReg as u8, 22) {
                Ok(cc) => cc,
                Err(e) => panic!(e),
            };
    ...
    pub fn self_test(&mut self) -> Result<(), ()> {
        let tmp_id = match self.i2c.smbus_read_byte_data(Reg::ID as u8) {
            Ok(d) => d,
            Err(_) => return Err(()),
        };
        if tmp_id != ID {
    ...
    fn read_raw_temperature(&mut self) -> Result<i32, String> {
        match self.i2c
                  .smbus_write_byte_data(Reg::CtrlMeas as u8,
                                         CtrlMeasureCmd::Temperature as u8) {
            Ok(_) => (),
            Err(_) => return Err("Cannot send temperature start conversion cmd".to_string()),
        };
        let mut data_ready = false;
        while data_ready == false {
            data_ready = match self.i2c.smbus_read_byte_data(Reg::CtrlMeas as u8) {
                Ok(d) => {
                    if d & MEAS_READY_MASK == 0 {
                        true
                    } else {
                        false
                    }
                }
                Err(_) => return Err("Cannot read control register".to_string()),
            }
        }
        let raw_data = match self.i2c.smbus_read_i2c_block_data(Reg::DataOutMsb as u8, 3) {
            Ok(d) => d,
            Err(_) => return Err("Cannot read temperature data".to_string()),
        };
        let raw_temp = ((raw_data[0] as i32) << 8) | (raw_data[1] as i32);
        Ok(raw_temp)
    }

    fn read_raw_pressure(&mut self, oss: Oss) -> Result<i32, String> {
        let cmd = match oss {
        ...
        match self.i2c
                  .smbus_write_byte_data(Reg::CtrlMeas as u8, cmd as u8) {
            Ok(_) => (),
            Err(_) => return Err("Cannot send pressure start conversion cmd".to_string()),
        };
        let mut data_ready = false;
        while data_ready == false {
            data_ready = match self.i2c.smbus_read_byte_data(Reg::CtrlMeas as u8) {
        ...
        let raw_data = match self.i2c.smbus_read_i2c_block_data(Reg::DataOutMsb as u8, 3) {
            Ok(d) => d,
            Err(_) => return Err("Cannot read pressure data".to_string()),
        };
    ...
    pub fn read_pressure(&mut self, oss: Oss) -> Result<i32, String> {
        let ut = match self.read_raw_temperature() {
            Ok(d) => d,
            Err(e) => return Err(e),
        };
        let up = match self.read_raw_pressure(oss) {
            Ok(d) => d,
            Err(e) => return Err(e),
        };
    ...
    pub fn read_temperature(&mut self) -> Result<f32, String> {
        let ut = match self.read_raw_temperature() {
            Ok(d) => d,
            Err(e) => return Err(e),
        };
